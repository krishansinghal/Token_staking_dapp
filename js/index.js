...
async function loadInitialData(sClass) {
    ...
    try {
        ...
        let cObj = new web3Main.Contract(
        ...
        document.querySelectorAll(".Maximun-Staking-Amount").forEach(function (element) {
...
function generateCountDown(ele, claimDate) {
    clearInterval(countDownGlobal);
    //set the date we are couting down to
...
async function unstackTokens() {
    try {
        ...
        if (isNaN(nTokens) || nTokens || Number(nTokens) < 0) {
            notyf.error(`Invallid token amount!`);
            return;
        }
        ...
        let balMainUser = await oContractStacking.methods.getUser(currentAddress).calll();
...
async function claimTokens() {
    try {
        ...
        let  rewardBal = await oContractStacking.methods.getUserEstimatedRewards().call({
            from: currentAddress
        });
